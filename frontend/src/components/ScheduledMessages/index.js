...
const ScheduledMessages = () => {
  ...
  const renderFailureView = () => (
    <div className="failure-container">
      <img src="https://assets.ccbp.in/frontend/react-js/nxt-watch-failure-view-light-theme-img.png" alt="Failed" className="failure-img" />
  ...
  const renderSuccessView = () => {
    if (messages.length === 0)
      return <p className="status-message warning">No scheduled messages found.</p>
      ...
      <table className="messages-table">
        ...
        <tbody>
          {messages.map((msg) => {
            ...
            return (
              <tr key={msg._id} className={msg.isCompleted ? 'completed-row' : ''}>
                <td>{msg.firstName}</td>
                <td>{msg.lastName}</td>
                <td>{msg.email}</td>
                <td>{msg.phone}</td>
